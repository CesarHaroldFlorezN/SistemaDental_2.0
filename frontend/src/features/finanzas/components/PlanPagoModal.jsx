...
export default function PlanPagoModal({
  ...
  const cuotasPreview = useMemo(() => {
    const cantidad = Math.max(1, Number.parseInt(formData.nCuotas, 10) || 1);
    ...
    return Array.from({ length: cantidad }, (_, indice) => {
      const sesion = formData.sesiones[indice] || null;
      return {
        num: indice + 1,
        tipo: 'cuota',
        ...
        fechaPago: null,
        metodoPago: null,
        sesionPlanId: esTratamiento ? sesion?.id || null : null,
        sesionNum: esTratamiento ? sesion?.numero || indice + 1 : null
      };
    });
  }, [esTratamiento, formData.fechaPrimeraCuota, formData.intervalo, formData.nCuotas, formData.sesiones, saldoFinanciar]);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!formData.pacienteId || !formData.pagoId || total <= 0 || saldoFinanciar <= 0) return;
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-slate-700 bg-slate-800 shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-700 px-6 py-4">
          <div>
            <h2 className="flex items-center gap-2 text-xl font-bold text-cyan-400"><CreditCard size={20} /> Crear plan de pago en cuotas</h2>
            <p className="mt-1 text-xs text-slate-500">La deuda queda vinculada a su origen clínico, sin volver a escribir paciente ni monto.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 overflow-y-auto p-6 text-sm">
          {datosIniciales && (
            <div className="flex items-start gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-emerald-100">
              ...
              <div><strong>Vinculación automática activa.</strong><div className="mt-0.5 text-xs text-slate-400">Este cronograma actualizará la deuda de {esTratamiento ? 'todo el plan de tratamiento' : 'esta atención puntual'}.</div></div>
            </div>
          )}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="font-medium text-slate-300">Concepto
              <input name="concepto" required value={formData.concepto} onChange={handleChange} readOnly={Boolean(datosIniciales?.concepto)} className="mt-1.5 w-full rounded-xl border border-slate-700 bg-slate-900 px-3.5 py-2.5 text-white outline-none focus:border-cyan-500 read-only:opacity-70" />
            </label>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <label className="font-medium text-slate-300">Número de cuotas
              <input type="number" min="1" max="60" name="nCuotas" disabled={esTratamiento} value={formData.nCuotas} onChange={handleChange} className="mt-1.5 w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2.5 text-white outline-none focus:border-cyan-500 disabled:opacity-60" />
              <span className="mt-1 block text-[11px] text-slate-500">{esTratamiento ? 'Una cuota por cada sesión del tratamiento.' : 'Libre para procedimientos pequeños.'}</span>
            </label>
          ...
          <div>
            <div className="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">Cronograma ({cuotasPreview.length} cuotas)</div>
            <div className="max-h-56 overflow-y-auto rounded-xl border border-slate-700 bg-slate-900/80">
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-800">{cuotasPreview.map((cuota) => <tr key={cuota.num}><td className="p-3 font-bold text-cyan-400">#{cuota.num}</td>{esTratamiento && <td className="p-3 text-purple-300">Sesión {cuota.sesionNum}</td>}<td className="p-3 text-slate-300">{cuota.fecha}</td><td className="p-3 text-right font-bold text-white">S/. {cuota.monto.toFixed(2)}</td></tr>)}</tbody>
              </table>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 border-t border-slate-700 pt-4">
            ...
            <button type="submit" disabled={guardando || saldoFinanciar <= 0 || !paciente || !formData.pagoId} className="flex items-center gap-2 rounded-xl bg-cyan-600 px-5 py-2.5 font-semibold text-white hover:bg-cyan-500 disabled:opacity-50"><Save size={18} />{guardando ? 'Guardando...' : 'Crear cronograma vinculado'}</button>
          </div>
        </form>
      </div>
    </div>
