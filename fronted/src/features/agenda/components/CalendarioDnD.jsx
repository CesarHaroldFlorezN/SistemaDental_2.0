...
export default function CalendarioDnD({
  ...
  useEffect(() => {
    ...
    const mover = (event) => {
      ...
      const movido = actual.movido || distancia > 6;
      ...
      if (actual.modo === 'mover') {
        inicioMinuto = minutoDesdeY(event.clientY, columna.rect) - Number(actual.desfaseMinutos || 0);
        inicioMinuto = Math.round(inicioMinuto / step) * step;
    ...
    const terminar = async (event) => {
      ...
      if (!actual.movido) {
        if (actual.modo === 'mover') onSelectEvent?.(actual.event, event);
        return;
      }
  ...
  const iniciar = (eventPointer, event, modo, nodoEvento = null) => {
    ...
    const duracionEvento = Math.max(step, Math.round((event.end - event.start) / 60000));
    const rectEvento = nodoEvento?.getBoundingClientRect?.();
    const proporcionAgarre = rectEvento?.height
      ? Math.max(0, Math.min(1, (eventPointer.clientY - rectEvento.top) / rectEvento.height))
      : 0;
    const desfaseMinutos = modo === 'mover'
      ? Math.round((duracionEvento * proporcionAgarre) / step) * step
      : 0;
    const siguiente = {
      event,
      modo,
  ...
  const iniciarDesdeTarjeta = (eventPointer) => {
    if (eventPointer.button !== 0 || view === 'month') return;
    const objetivo = eventPointer.target instanceof Element ? eventPointer.target : null;
    const nodoEvento = objetivo?.closest('.rbc-event');
    if (!nodoEvento || !raizRef.current?.contains(nodoEvento)) return;

    const nodoIdentificador = nodoEvento.querySelector('[data-dp-event-id]');
    const identificador = nodoIdentificador?.getAttribute('data-dp-event-id');
    const evento = (calendarProps.events || []).find((item) => String(item.id) === String(identificador));
    if (!evento) return;

    const modo = objetivo?.closest('.dp-control-duracion') ? 'redimensionar' : 'mover';
    iniciar(eventPointer, evento, modo, nodoEvento);
  };
  ...
  const EventoInteractivo = useMemo(() => function EventoInteractivoInterno(props) {
    ...
    return (
      <div
        data-dp-event-id={props.event.id}
  ...
  return (
    <div ref={raizRef} className="dp-calendario-interactivo relative" onPointerDownCapture={iniciarDesdeTarjeta}>
      <Calendar
        {...calendarProps}
      ...
      {puntero && interaccion && (
        <div className="pointer-events-none fixed z-[9999] max-w-[220px] -translate-x-1/2 -translate-y-[calc(100%+14px)] rounded-xl border border-cyan-300/60 bg-slate-900/95 px-3 py-2 text-xs font-bold text-white shadow-2xl" style={{ left: puntero.x, top: puntero.y }}>
          {puntero.texto}
          <div className="mt-0.5 text-[10px] font-medium text-cyan-300">{interaccion.modo === 'redimensionar' ? 'Cambiando duración' : 'Moviendo cita'}</div>
        </div>
      )}
    </div>
  );
