...
function Evento({ event, editable }) {
  ...
  const listaServicios = servicios(cita);
  const principal = listaServicios[0]?.nombre || 'Consulta';
  const adicionales = Math.max(0, listaServicios.length - 1);
  const duracion = Math.max(15, Math.round((event.end - event.start) / 60000));
  const info = ESTADOS[estadoVisual(cita.estado)] || ESTADOS.pendiente;

  return (
    <div className={`relative flex h-full min-w-0 flex-col overflow-hidden px-0.5 leading-tight ${editable ? 'cursor-move' : 'cursor-pointer'}`}>
      <div className="flex items-center justify-between gap-1 text-[9px] font-black uppercase tracking-wide text-slate-200/90">
        <span className="truncate">{cita.hora} - {horaFin(cita)}</span>
        {editable ? <Move size={11} className="shrink-0 opacity-80" /> : <span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ backgroundColor: info.color }} />}
      </div>
      <div className="mt-0.5 truncate text-[12px] font-black text-white">{cita.nombrePaciente}</div>
      {duracion >= 45 && <div className="mt-0.5 truncate text-[10px] font-semibold text-slate-100/90">{principal}{adicionales ? ` +${adicionales}` : ''}</div>}
      {duracion >= 75 && cita.codigoFicha && <div className="mt-auto truncate pt-1 text-[9px] font-bold text-slate-200/70">Ficha {cita.codigoFicha}</div>}
    </div>
  );
}
...
export default function AgendaClinicaProfesionalV8({
  ...
  const propiedadesEvento = (event) => {
    ...
    return {
      style: {
        ...
        color: '#f8fafc',
        border: `1px solid ${info.color}aa`,
        borderLeft: `4px solid ${info.color}`,
        borderRadius: 8,
        padding: '3px 5px',
        boxShadow: edicionHorarios ? `0 0 0 2px ${info.color}45, 0 8px 18px rgba(15,23,42,.32)` : '0 3px 10px rgba(15,23,42,.22)',
        cursor: edicionHorarios ? 'move' : 'pointer'
      }
    };
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-700 bg-slate-800/80 p-3">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
          <div className="flex rounded-xl border border-slate-700 bg-slate-900 p-1">
            ...
            ].map(([valor, texto, icono]) => <button key={valor} type="button" onClick={() => setModo(valor)} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-xs font-bold ${modo === valor ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white'}`}>{icono}{texto}</button>)}
          </div>

          {modo !== 'recepcion' && <>
            <div className="relative min-w-[240px] flex-1">
              <Search size={17} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500" />
              <input type="search" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} placeholder="Paciente, DNI, ficha, teléfono o servicio..." className="w-full rounded-xl border border-slate-700 bg-slate-900 py-2.5 pl-10 pr-9 text-sm text-white outline-none focus:border-cyan-500" />
            ...
            <div className="relative min-w-[170px]">
              ...
              <select value={estadoFiltro} onChange={(e) => setEstadoFiltro(e.target.value)} className="w-full appearance-none rounded-xl border border-slate-700 bg-slate-900 py-2.5 pl-9 pr-8 text-sm text-white outline-none focus:border-cyan-500">
            ...
            <div className="relative min-w-[170px]">
              <select value={rangoFecha} onChange={(e) => { setRangoFecha(e.target.value); setFechaExacta(''); }} className="w-full appearance-none rounded-xl border border-slate-700 bg-slate-900 py-2.5 pl-3 pr-8 text-sm text-white outline-none focus:border-cyan-500">
