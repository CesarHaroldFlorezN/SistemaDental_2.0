...
export default function AgendaClinicaProfesional({
  ...
  const confirmarMovimientoCalendario = async ({ event, start, end }) => {
    if (!moverCalendarioActivo || event?.esResumenMes) return;
    const cita = event.citaData;
    const duracion = obtenerDuracionCita(cita);
    const finDestino = end instanceof Date && end > start
      ? end
      : new Date(start.getTime() + duracion * 60 * 1000);

    const confirmacion = await Swal.fire({
      title: '¿Mover esta cita?',
      html: `<div style="text-align:left;line-height:1.7"><strong>${cita.nombrePaciente}</strong><br>${format(start, 'dd/MM/yyyy')}<br>${format(start, 'HH:mm')} → ${format(finDestino, 'HH:mm')}</div>`,
      icon: 'question',
      ...
      {modoVista === 'calendario' && (
        <div className="agenda-calendario-pro rounded-2xl border border-slate-700/80 bg-slate-800/80 p-4 shadow-xl">
          <style>{`
            ...
            .agenda-calendario-pro .rbc-addons-dnd .rbc-addons-dnd-drag-preview,
            .agenda-calendario-pro .rbc-addons-dnd-drag-preview { opacity: .88; border: 2px dashed #67e8f9 !important; border-radius: 12px !important; box-shadow: 0 14px 32px rgba(8,145,178,.32) !important; }
            .agenda-calendario-pro .rbc-addons-dnd-over { background: rgba(86,117,158,.18) !important; box-shadow: inset 0 0 0 2px rgba(141,159,193,.55); }
            .agenda-calendario-pro .rbc-addons-dnd-dragged-event { opacity: .45; }
          ...
          <div className="mb-4 flex flex-col gap-3 rounded-2xl border border-slate-700 bg-slate-900/55 p-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <label className={`flex cursor-pointer items-center gap-3 rounded-xl border px-3 py-2 transition ${moverCalendarioActivo ? 'border-cyan-500 bg-cyan-500/10' : 'border-slate-700 bg-slate-900'}`}>
              ...
              <div><div className={`text-xs font-bold ${moverCalendarioActivo ? 'text-cyan-300' : 'text-slate-300'}`}>Permitir mover citas</div><div className="text-[10px] text-slate-500">Arrastra la cita; la sombra indica dónde quedará</div></div>
            </label>
          </div>

          {moverCalendarioActivo && (
            <div className="mb-3 rounded-xl border border-cyan-500/40 bg-cyan-500/10 px-3 py-2 text-xs text-cyan-200">
              Modo mover activo: toma una cita y arrástrala libremente. Antes de guardar se solicitará confirmación.
            </div>
          )}
          ...
          <div style={{ height: '76vh', minHeight: 640 }}>
            <DragAndDropCalendar
              ...
              popup
              resizable={false}
              draggableAccessor={(event) => moverCalendarioActivo && vistaCalendario !== 'month' && !event.esResumenMes}
              eventPropGetter={(event) => {
                ...
                return { style: { backgroundColor: ESTADOS[estado]?.colorCalendario || '#64748b', color: '#fff', border: '1px solid rgba(255,255,255,.18)', fontSize: '12px', padding: '4px 6px', boxShadow: '0 5px 12px rgba(0,0,0,.22)' } };
              }}
              ...
              onEventDrop={confirmarMovimientoCalendario}
