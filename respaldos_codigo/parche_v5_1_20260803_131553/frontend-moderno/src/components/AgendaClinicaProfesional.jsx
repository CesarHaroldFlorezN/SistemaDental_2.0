  ...
  return (
    <article
      ...
      className={`overflow-hidden rounded-2xl border bg-gradient-to-b from-slate-900/95 to-slate-900/70 shadow-lg transition hover:-translate-y-0.5 hover:shadow-xl ${
        cita.estado === 'en_atencion' ? 'border-rose-500/50' : 'border-slate-700'
      } ${arrastrando ? 'scale-[0.98] opacity-45' : ''}`}
    >
      <div className="p-3.5">
        <div className="grid grid-cols-[58px_minmax(132px,1fr)_34px] items-center gap-2">
          <div className="rounded-lg border border-slate-600 bg-slate-800/90 px-2 py-1.5 text-center shadow-inner">
            <div className="text-[8px] font-black uppercase tracking-wider text-slate-500">Ficha</div>
            <div className="mt-0.5 truncate text-base font-black leading-none text-white">{cita.codigoFicha || '—'}</div>
          </div>
...
export default function AgendaClinicaProfesional({
  ...
  const seleccionarSlotSeguro = (slotInfo) => {
    if (citaDetalle || moverCalendarioActivo || Date.now() < bloquearSeleccionRef.current) return;
  ...
  const moverEventoCalendario = async ({ event, start }) => {
    if (!moverCalendarioActivo) return;
    const cita = event.citaData;
    const nuevaFecha = format(start, 'dd/MM/yyyy');
    const nuevaHora = format(start, 'HH:mm');
    const fin = new Date(start.getTime() + obtenerDuracionCita(cita) * 60 * 1000);

    const confirmacion = await Swal.fire({
      title: '¿Mover esta cita?',
      html: `<div style="text-align:left;line-height:1.6"><strong>${cita.nombrePaciente}</strong><br>${nuevaFecha}<br>${nuevaHora} → ${format(fin, 'HH:mm')}</div>`,
      icon: 'question',
    ...
    if (confirmacion.isConfirmed) {
      await onReprogramarCita?.(cita, start);
    }
  };
        ...
        <div className="agenda-calendario-pro rounded-2xl border border-slate-700/80 bg-slate-800/80 p-4 shadow-xl">
          ...
          <div className="mb-4 flex flex-col gap-3 rounded-2xl border border-slate-700 bg-slate-900/55 p-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <label className={`flex cursor-pointer items-center gap-3 rounded-xl border px-3 py-2 transition ${moverCalendarioActivo ? 'border-cyan-500 bg-cyan-500/10' : 'border-slate-700 bg-slate-900'}`}>
              <input type="checkbox" checked={moverCalendarioActivo} onChange={(event) => setMoverCalendarioActivo(event.target.checked)} className="h-4 w-4 accent-cyan-500" />
              <div><div className={`text-xs font-bold ${moverCalendarioActivo ? 'text-cyan-300' : 'text-slate-300'}`}>Permitir mover citas</div><div className="text-[10px] text-slate-500">Actívalo antes de arrastrar en el calendario</div></div>
            </label>
          </div>
          ...
          <div style={{ height: '76vh', minHeight: 640 }}>
            <DragAndDropCalendar
              ...
              components={{ event: EventoCalendario, toolbar: ToolbarCalendario }}
              messages={{ next: 'Siguiente', previous: 'Anterior', today: 'Hoy', month: 'Mes', week: 'Semana', day: 'Día', agenda: 'Lista', date: 'Fecha', time: 'Hora', event: 'Paciente / tratamiento', noEventsInRange: 'No hay citas en este rango.' }}
              step={15}
              timeslots={4}
              min={new Date(1970, 0, 1, 7, 0, 0)}
              max={new Date(1970, 0, 1, 21, 0, 0)}
              popup
              eventPropGetter={(event) => ({ style: { backgroundColor: ESTADOS[event.estado]?.colorCalendario || '#475569', color: '#fff', border: 'none', fontSize: '12px', padding: '4px 6px', boxShadow: '0 5px 12px rgba(0,0,0,.22)' } })}
              selectable={!citaDetalle && !moverCalendarioActivo}
              draggableAccessor={() => moverCalendarioActivo}
              resizable={false}
              onEventDrop={moverEventoCalendario}
