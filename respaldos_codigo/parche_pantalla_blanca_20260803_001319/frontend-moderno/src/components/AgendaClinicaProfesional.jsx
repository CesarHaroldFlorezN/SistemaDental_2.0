...
const CalendarioArrastrable = withDragAndDrop(Calendar);
      ...
      {modoVista === 'calendario' && (
        <div className="rounded-2xl border border-slate-700/80 bg-slate-800/80 p-4 shadow-xl">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-xs text-slate-400"><GripVertical size={15} /> Arrastra una cita para reprogramarla. Haz clic para ver acciones rápidas.</div>
          ...
          <div style={{ height: '72vh' }}>
            <CalendarioArrastrable
              ...
              selectable
              resizable={false}
              draggableAccessor={(event) => ESTADOS_ACTIVOS.has(event.estado)}
              onEventDrop={({ event, start, end }) => onReprogramarCita?.(event.citaData, start, end)}
