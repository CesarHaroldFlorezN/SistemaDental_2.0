...
const ESTADOS = {
  ...
  en_atencion: { texto: 'En atencion', color: '#f43f5e', clase: 'border-rose-500/60' },
  completada: { texto: 'Finalizada', color: '#10b981', clase: 'border-emerald-500/55' },
  no_asistio: { texto: 'No asistio', color: '#ea580c', clase: 'border-orange-500/55' },
...
function Evento({ event, editable }) {
  return (
    <div className={`relative min-w-0 leading-tight ${editable ? 'cursor-move' : ''}`}>
      {editable && <Move size={12} className="absolute right-0 top-0 opacity-70" />}
      <div className="truncate pr-4 font-bold">{event.citaData.nombrePaciente}</div>
      <div className="truncate text-[10px] opacity-90">{event.citaData.hora} - {horaFin(event.citaData)}</div>
      <div className="truncate text-[10px] opacity-85">{servicios(event.citaData).map((s) => s.nombre).join(' + ')}</div>
    </div>
  );
}
...
export default function AgendaClinicaProfesionalV8({
  citas = [], pacientes = [], pagos = [], onNuevaCita, onEditarCita, onCambiarEstado,
  onCompletarCita, onCancelarCita, onCobrar, onVerFicha, onReprogramarCita, onVerCuotas
}) {
  ...
  const enriquecidas = useMemo(() => citas.map((cita) => {
    ...
    return { ...cita, nombrePaciente: paciente?.nombre || cita.nombrePaciente || 'Paciente', codigoFicha: paciente?.codigo_ficha || cita.codigoFicha || '', pago };
  }), [citas, pacientes, pagos]);

  const filtradas = useMemo(() => {
    const q = busqueda.trim().toLowerCase();
    if (!q) return enriquecidas;
    return enriquecidas.filter((cita) => [cita.nombrePaciente, cita.codigoFicha, cita.procedimiento, ...servicios(cita).map((s) => s.nombre)].join(' ').toLowerCase().includes(q));
  }, [enriquecidas, busqueda]);
  ...
  const callbacks = { onEditar: onEditarCita, onCambiarEstado, onCompletar: onCompletarCita, onCancelar: onCancelarCita, onCobrar, onVerCuotas };
  ...
  const abrirEvento = (event) => {
    ...
    if (!edicionHorarios) setDetalle(event.citaData);
  };
  ...
  const propiedadesEvento = (event) => {
    ...
    return { style: { backgroundColor: info.color, borderColor: info.color, borderRadius: 8, boxShadow: edicionHorarios ? `0 0 0 2px ${info.color}55` : 'none', cursor: edicionHorarios ? 'move' : 'pointer' } };
  };
  ...
  const calendarioComun = {
    ...
    messages: { today: 'Hoy', previous: 'Anterior', next: 'Siguiente', month: 'Mes', week: 'Semana', day: 'Dia', noEventsInRange: 'No hay citas.' },
    style: { height: 720 }
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <div><h1 className="text-3xl font-bold text-cyan-400">Agenda y recepcion clinica</h1><p className="mt-1 text-sm text-slate-400">Gestiona pacientes, horarios y cobros desde una sola vista.</p></div>
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-700 bg-slate-800/80 p-3 lg:flex-row lg:items-center">
        <div className="flex rounded-xl border border-slate-700 bg-slate-900 p-1">
          {[
            ['recepcion', 'Recepcion', <LayoutDashboard size={16} />],
            ['calendario', 'Calendario', <CalendarDays size={16} />],
            ['resumen', 'Resumen', <LayoutList size={16} />]
          ].map(([valor, texto, icono]) => <button key={valor} type="button" onClick={() => setModo(valor)} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2 text-xs font-bold ${modo === valor ? 'bg-cyan-600 text-white' : 'text-slate-400'}`}>{icono}{texto}</button>)}
        </div>
        {modo !== 'recepcion' && <div className="relative flex-1"><Search size={17} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500" /><input type="search" value={busqueda} onChange={(e) => setBusqueda(e.target.value)} placeholder="Buscar paciente, ficha o servicio..." className="w-full rounded-xl border border-slate-700 bg-slate-900 py-2.5 pl-10 pr-4 text-sm text-white outline-none focus:border-cyan-500" /></div>}
      </div>
      ...
      {modo === 'recepcion' && <>
        <div className="flex items-center justify-between rounded-2xl border border-slate-700 bg-slate-800/60 p-3">
          ...
          <div className="text-center"><div className="text-xs font-bold uppercase text-cyan-400">Recepcion del dia</div><input type="date" value={fechaRecepcion} onChange={(e) => setFechaRecepcion(e.target.value)} className="mt-1 rounded-lg border border-slate-700 bg-slate-900 px-3 py-1.5 text-sm font-bold text-white" /></div>
        ...
        <div className="grid gap-4 xl:grid-cols-4">
          ...
          <Columna titulo="En atencion" estado="en_atencion" citas={columnas.en_atencion} callbacks={callbacks} onDetalle={setDetalle} arrastre={arrastre} />
      ...
      {modo === 'calendario' && <section className="rounded-2xl border border-slate-700 bg-slate-800/70 p-4 shadow-xl">
        <div className="mb-4 flex flex-col gap-3 rounded-xl border border-slate-700 bg-slate-900/60 p-3 md:flex-row md:items-center md:justify-between">
          <div><div className="flex items-center gap-2 font-bold text-white"><CalendarCheck2 size={18} className="text-cyan-400" />Calendario clinico</div><div className="mt-1 text-xs text-slate-500">Mueve la tarjeta para cambiar dia/hora. Arrastra el borde inferior para cambiar la duracion.</div></div>
      ...
      {modo === 'resumen' && <section className="rounded-2xl border border-slate-700 bg-slate-800/70 p-4">
        <div className="mb-4 grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {['pendiente', 'en_espera', 'en_atencion', 'completada', 'no_asistio', 'cancelada'].map((estado) => <div key={estado} className="rounded-xl border border-slate-700 bg-slate-900/60 p-3"><div className="text-[10px] uppercase text-slate-500">{ESTADOS[estado].texto}</div><div className="mt-1 text-2xl font-black" style={{ color: ESTADOS[estado].color }}>{filtradas.filter((c) => estadoVisual(c.estado) === estado).length}</div></div>)}
        </div>
        <div className="space-y-2">{filtradas.sort((a, b) => `${b.fecha}${b.hora}`.localeCompare(`${a.fecha}${a.hora}`)).map((cita) => <button key={cita.id} type="button" onClick={() => setDetalle(cita)} className="grid w-full gap-2 rounded-xl border border-slate-700 bg-slate-900/60 p-3 text-left sm:grid-cols-[120px_minmax(0,1fr)_160px]"><span className="text-xs font-bold text-cyan-300">{cita.fecha}<br />{cita.hora} - {horaFin(cita)}</span><span><span className="font-bold text-white">{cita.nombrePaciente}</span><span className="mt-1 block text-xs text-slate-400">{servicios(cita).map((s) => s.nombre).join(' + ')}</span></span><span className="text-right text-xs" style={{ color: ESTADOS[estadoVisual(cita.estado)].color }}>{ESTADOS[estadoVisual(cita.estado)].texto}</span></button>)}</div>
      </section>}

      {detalle && <div className="fixed inset-0 z-[85] flex items-center justify-center bg-black/70 p-4" onClick={() => setDetalle(null)}><div className="w-full max-w-xl rounded-2xl border border-slate-700 bg-slate-800 p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}><div className="flex items-start justify-between"><div><div className="text-xs font-bold uppercase text-cyan-400">Detalle de cita</div><h2 className="mt-1 text-xl font-black text-white">{detalle.nombrePaciente}</h2></div><button type="button" onClick={() => setDetalle(null)} className="text-slate-400"><XCircle size={20} /></button></div><div className="mt-4 space-y-2 text-sm text-slate-300"><div><Clock3 size={15} className="mr-2 inline text-cyan-400" />{detalle.fecha} · {detalle.hora} - {horaFin(detalle)}</div>{servicios(detalle).map((s, i) => <div key={i} className="flex justify-between rounded-lg border border-slate-700 bg-slate-900/60 px-3 py-2"><span>{s.nombre}</span><span className="font-bold text-cyan-300">{moneda(s.costo)}</span></div>)}</div><div className="mt-4 flex flex-wrap gap-2">{detalle.estado !== 'completada' && <button type="button" onClick={() => { onEditarCita?.(detalle); setDetalle(null); }} className="rounded-xl bg-cyan-600 px-4 py-2 text-xs font-bold text-white">Editar cita</button>}<button type="button" onClick={() => { const p = pacientes.find((item) => Number(item.id) === Number(detalle.pacienteId)); if (p) onVerFicha?.(p); setDetalle(null); }} className="rounded-xl border border-slate-600 px-4 py-2 text-xs font-bold text-slate-200">{detalle.estado === 'completada' ? 'Ver cierre, cuenta y pagos' : 'Ver ficha'}</button>{detalle.estado === 'completada' && <div className="w-full rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 text-xs text-emerald-100">La atencion finalizada no cambia de estado desde la agenda. Las correcciones de cobro se registran como anulacion o devolucion en la ficha del paciente.</div>}</div></div></div>}
      ...
      <style>{`
        ...
        .dp-control-duracion { position: absolute; left: 0; right: 0; bottom: -4px; height: 12px; cursor: ns-resize; display: flex; align-items: flex-end; justify-content: center; background: transparent; border: 0; padding: 0; touch-action: none; }
        .dp-control-duracion span { width: 30px; height: 4px; border-radius: 999px; background: rgba(255,255,255,.9); box-shadow: 0 1px 4px rgba(0,0,0,.35); }
