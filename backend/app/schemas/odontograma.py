...
PIEZAS_PERMANENTES = {
    f"{cuadrante}{pieza}" for cuadrante in (1, 2, 3, 4) for pieza in range(1, 9)
}
PIEZAS_DECIDUAS = {
    f"{cuadrante}{pieza}" for cuadrante in (5, 6, 7, 8) for pieza in range(1, 6)
}
...
class OdontogramaPayload(BaseModel):
    ...
    hallazgos: list[HallazgoOdontogramaPayload] = Field(
    ...
    def validar_piezas_de_denticion(self) -> "OdontogramaPayload":
        piezas = {pieza for hallazgo in self.hallazgos for pieza in hallazgo.piezas}
