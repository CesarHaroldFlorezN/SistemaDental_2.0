...
def test_subir_listar_descargar_y_eliminar_documento() -> None:
    ...
    assert any(item["id"] == documento_id for item in respuesta_listar.json())
    ...
    assert respuesta_eliminar.json()["message"] == "Documento eliminado."

    respuesta_listar_final = client.get(f"/api/pacientes/{paciente_id}/documentos")

    assert respuesta_listar_final.status_code == 200
    assert not any(item["id"] == documento_id for item in respuesta_listar_final.json())
